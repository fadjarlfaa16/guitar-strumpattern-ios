...
final class ChordAnalyzer {
    ...
    func analyze(audioURL: URL, onProgress: (@Sendable (Double, AnalysisStage) -> Void)? = nil) throws -> ChordAnalysisResult {
        // Step 1: Load Audio
        onProgress?(0.08, .loadingAudio)
        ...
        onProgress?(0.48, .buildingMatrix)
        ...
        for (i, modelName) in Self.ensembleModelNames.enumerated() {
            try Task.checkCancellation()
            onProgress?(0.52 + Double(i) * 0.06, .neuralNet)
        ...
        onProgress?(0.80, .combiningOutputs)
        ...
        onProgress?(0.84, .parsingPredictions)
        ...
        // Step 8: HMM decode
        onProgress?(0.88, .decodingSequences)
        ...
        onProgress?(0.93, .buildingTimeline)
